...
## Same as sklearn_rf_training.py, but each CV prediction is saved in a file *_preds.txt with the following format:
## header (sample names)
## as many rows as CV * FSTEPS iterations
## as many cols as samples
...
PREDS = np.zeros((CV_K*CV_N*len(FSTEPS), len(sample_names)), dtype=np.int)
PROBA0 = -10 * np.ones((CV_K*CV_N*len(FSTEPS), len(sample_names)), dtype=np.float)
PROBA1 = -10 * np.ones_like(PROBA0)
...
for n in range(CV_N):
    ...
    for i, (idx_tr, idx_ts) in enumerate(idx):
        ...
        for j, s in enumerate(FSTEPS):
            ...
            proba = forest.predict_proba(x_ts_fs)
            PREDS[len(FSTEPS)*(n*CV_K + i) + j, idx_ts] = p
            PROBA0[len(FSTEPS)*(n*CV_K + i) + j, idx_ts] = proba[:, 0]
            PROBA1[len(FSTEPS)*(n*CV_K + i) + j, idx_ts] = proba[:, 1]
...
probaf = open(OUTFILE + "_proba0.txt", 'w')
probaf_w = csv.writer(probaf, delimiter='\t', lineterminator='\n')
probaf_w.writerow(sample_names)
for row in PROBA0:
    probaf_w.writerow(row.tolist())
probaf.close()

probaf = open(OUTFILE + "_proba1.txt", 'w')
probaf_w = csv.writer(probaf, delimiter='\t', lineterminator='\n')
probaf_w.writerow(sample_names)
for row in PROBA1:
    probaf_w.writerow(row.tolist())
