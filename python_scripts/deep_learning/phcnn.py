...
def data():
    ...
    from keras.layers import Dense, Activation, Dropout, Convolution2D, Flatten, MaxPooling2D, ZeroPadding2D
...
def model(input_file):
    ...
    def phyloneighbors(x, coordinates,k):
        ...
        for col in range(x.shape[1]*k):
                res=int((col%k))
                j = int((col/k))
                #print 'col',col, 'j',j, 'res', res
                #print neighbors[j,res]
                output[:,col] =  x[:,neighbors[j,res]]
        output = np.reshape(output, (output.shape[0],1,1,output.shape[1]))
        return output
    ...
    k = {{choice([2, 3, 4, 5])}}
    nb_filter = {{choice([2,3,4])}}
    trial_model = Sequential()

    trial_model.add(Convolution2D(input_shape=(1,1,x.shape[1]*k), nb_filter=nb_filter, nb_row=1, nb_col=k, border_mode='valid', activation='relu', subsample=(1,k)))
    trial_model.add(ZeroPadding2D(padding=(0, 1)))
    trial_model.add(Cropping2D(cropping=((0, 0), (1, 0))))
    trial_model.add(MaxPooling2D(pool_size=(1, 2), border_mode="valid"))
    trial_model.add(Flatten())


    # If we choose 'four', add an additional fourth layer
    if conditional({{choice(['three', 'four'])}}) == 'four':
        trial_model.add(Dense({{choice([32, 64, 128])}}))
    ...
    if trial_model_optimizer_list == 'adam':
        ...
        beta_1 = 0.9
        beta_2 = 0.999
        trial_model_optimizer = Adam(lr=0.001, beta_1=beta_1, beta_2=beta_2,epsilon=epsilon )
        trial_model_optimizer_dict['adam'] = {'lr': 0.001,
                                              'beta_1': beta_1,
                                              'beta_2': beta_2,
                                              'epsilon': epsilon}
        ...
        lr = 0.001
        rho = 0.9
        trial_model_optimizer = RMSprop(lr=0.001, rho=rho, epsilon=epsilon)
        trial_model_optimizer_dict['rmsprop'] = {'lr': lr,
                                              'rho': rho,
                                              'epsilon': epsilon}
    elif trial_model_optimizer_list == 'sgd':
        nesterov = True
        lr = {{choice([0.01 , 0.001])}}
        momentum = 0.9
        decay = 1e-06
        ...
        trial_model_optimizer_dict['sgd'] = {'lr': lr,
                                              ...
                                              'decay': decay,
                                              'nesterov': nesterov}
    trial_model_batch_size = {{choice([1, 4, 8, 16])}}
    ...
    for n in range(CV_N):
        ...
        for i, (idx_tr, idx_ts) in enumerate(idx):
            ...
            x_tr= phyloneighbors(x=x_tr, coordinates= coordinates,k=k)
            x_ts= phyloneighbors(x=x_ts, coordinates= coordinates,k=k)
...
while len(var_names) >= 5:
    ...
    for n in range(CV_N):
        ...
        for i, (idx_tr, idx_ts) in enumerate(idx):
            ...
            x_tr= phyloneighbors(x=x_tr, coordinates=coordinates,k=best_model['k'])
    ...
    validation_model.fit(x_tr, y_tr_cat,
                         batch_size=best_model['batch_size'],
                         nb_epoch=200 ,
                         show_accuracy=True,
                         verbose=0)
    ...
    #print 'idx_coordinates', [(int(i)-1) for i in var_names]
